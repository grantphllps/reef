...
//            ROS_WARN_STREAM("After<<\n"<<imu);
	...
	//std::cout << "takeoffState.data is: " << (takeoffState.data == false) << "\n";
        if (!takeoffState.data && numberOfPropagations >= 10)
        {
            if (enableXY){
                xyEst.resetLandingState();
		//ROS_INFO("Reset XY Landing State");
	    }
            if (enableZ) {
            	zEst.resetLandingState();
		//ROS_INFO("Rest Z Landing State");
	    }
	   ...
	   // ROS_INFO("Skipped the Reset =)");
	}
	//ROS_INFO("Left Reset Condition");
...
//                            ROS_WARN_STREAM("trueeeeee");
	...
	std::cout << "accTakeoffState " << accTakeoffState << "  ";
	std::cout << "sonarTakeoffState " << sonarTakeoffState <<  "  ";
	std::cout << "!takeoffState.data " << !takeoffState.data << "\n";
